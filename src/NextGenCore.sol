...
import "./openzeppelin/libs/Strings.sol";
import "./openzeppelin/libs/Base64.sol";
import "./openzeppelin/ERC2981.sol";
import "./Interfaces/IRandomizer.sol";
import "./Interfaces/INextGenAdmins.sol";
import "./Interfaces/IMinterContract.sol";
...
contract NextGenCore is ERC721Enumerable, Ownable, ERC2981 {
    ...
    mapping(uint256 => collectionAdditonalDataStructure)
        ...
        CollectionAdminRequired(_collectionID, this.setCollectionData.selector)
    {
        require(
            (isCollectionCreated[_collectionID] == true) &&
                ...
                (_collectionTotalSupply <= 10_000_000_000),
            "err/freezed"
        );
        if (
            ...
            collectionAdditionalData[_collectionID]
                .reservedMinTokensIndex = (_collectionID * 10_000_000_000);
            collectionAdditionalData[_collectionID].reservedMaxTokensIndex =
                (_collectionID * 10_000_000_000) +
    ...
    function burnToMint(
        ...
        require(
            _isApprovedOrOwner(burner, _tokenId),
            "ERC721: caller is not token owner or approved"
        );
        collectionAdditionalData[_mintCollectionID]
            .collectionCirculationSupply =
            collectionAdditionalData[_mintCollectionID]
                .collectionCirculationSupply +
            1;
    ...
    function _mintProcessing(
        ...
        collectionAdditionalData[_collectionID].randomizer.calculateTokenHash(
            _collectionID,
            _mintIndex,
            _saltfun_o
        );
    ...
    function changeMetadataView(
        ...
        public
        CollectionAdminRequired(_collectionID, this.changeMetadataView.selector)
    {
        require(
            (isCollectionCreated[_collectionID] == true) &&
                (collectionFreeze[_collectionID] == false),
    ...
    ) public FunctionAdminRequired(this.changeTokenData.selector) {
        require(
            collectionFreeze[tokenIdsToCollectionIds[_tokenId]] == false,
            "Data frozen"
        );
    ...
    function updateImagesAndAttributes(
        ...
        for (uint256 i; i < _tokenId.length; i++) {
            require(
                collectionFreeze[tokenIdsToCollectionIds[_tokenId[i]]] == false,
                "Data frozen"
            );
            _requireMinted(_tokenId[i]);
            tokenImageAndAttributes[_tokenId[i]][0] = _images[i];
            tokenImageAndAttributes[_tokenId[i]][1] = _attributes[i];
        }
    }
    ...
    function freezeCollection(
        uint256 _collectionID
    ) public FunctionAdminRequired(this.freezeCollection.selector) {
    ...
    ) public FunctionAdminRequired(this.addMinterContract.selector) {
        require(
            IMinterContract(_minterContract).isMinterContract() == true,
            "Contract is not Minter"
        );
    ...
    function setDefaultRoyalties(
        ...
        uint96 _bps
    ) public FunctionAdminRequired(this.setDefaultRoyalties.selector) {
    ...
    function tokenURI(
        ...
        if (
            ...
            string memory baseURI = collectionInfo[
                tokenIdsToCollectionIds[tokenId]
            ].collectionBaseURI;
            return
            ...
            return
                bytes(baseURI).length > 0
                    ? string(abi.encodePacked(baseURI, "pending"))
                    : "";
        } else {
    ...
    function getTokenName(
        ...
        uint256 tok = tokenId -
            collectionAdditionalData[tokenIdsToCollectionIds[tokenId]]
                .reservedMinTokensIndex;
        return
            string(
                abi.encodePacked(
                    collectionInfo[viewColIDforTokenID(tokenId)].collectionName,
                    " #",
                    tok.toString()
                )
            );
    ...
    function collectionFreezeStatus(
        uint256 _collectionID
    ...
    function retrievewereDataAdded(
        uint256 _collectionID
    ...
    function retrieveTokensMintedALPerAddress(
        uint256 _collectionID,
        address _address
    ) external view returns (uint256) {
    ...
    function retrieveTokensMintedPublicPerAddress(
        uint256 _collectionID,
        address _address
    ) external view returns (uint256) {
    ...
    function retrieveTokensAirdroppedPerAddress(
        uint256 _collectionID,
        address _address
    ) public view returns (uint256) {
    ...
    function retrieveArtistAddress(
        uint256 _collectionID
    ...
    function retrieveCollectionAdditionalData(
        uint256 _collectionID
    ) public view returns (address, uint256, uint256, uint256, uint, address) {
        return (
            collectionAdditionalData[_collectionID].collectionArtistAddress,
    ...
    function totalSupplyOfCollection(
        ...
        return (collectionAdditionalData[_collectionID]
            .collectionCirculationSupply - burnAmount[_collectionID]);
    }

    // function to retrieve the token image uri and the attributes stored on-chain for a token id.

    function retrievetokenImageAndAttributes(
        uint256 _tokenId
    ) public view returns (string memory, string memory) {
        return (
            tokenImageAndAttributes[_tokenId][0],
            tokenImageAndAttributes[_tokenId][1]
        );
    }
}
