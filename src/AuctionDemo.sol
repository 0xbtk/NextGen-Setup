...
contract AuctionDemo is Ownable {
    ...
    modifier WinnerOrAdminRequired(uint256 _tokenId, bytes4 _selector) {
        require(
            msg.sender == returnHighestBidder(_tokenId) ||
    ...
    mapping(uint256 => auctionInfo[]) public auctionInfoData;
    ...
    function participateToAuction(uint256 _tokenid) public payable {
        ...
        "XXXXXXXXX");
        auctionInfo memory newBid = auctionInfo({
            bidder: msg.sender,
            bid: msg.value,
            status: true
        });
    ...
    function cancelAllBids(uint256 _tokenid) public {
        ...
        for (uint256 i = 0; i < auctionInfoData[_tokenid].length; i++) {
            if (
                auctionInfoData[_tokenid][i].bidder == msg.sender &&
                auctionInfoData[_tokenid][i].status == true
            ) {
